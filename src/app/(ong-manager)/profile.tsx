...
export default function OngManagerProfileScreen() {
  ...
  const fullName = [userContext?.firstName, userContext?.lastName].filter(Boolean).join(' ') || 'Responsable ONG';
    ...
    <SafeAreaView edges={['bottom']} style={[styles.safeArea, { backgroundColor: colors.bgSecondary }]}>
      ...
      >
        ...
        <View style={[styles.profileCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.avatarContainer}>
            <View style={styles.avatar}>
              <Text style={styles.avatarInitials}>
                {(userContext?.firstName?.[0] || 'R') + (userContext?.lastName?.[0] || 'O')}
              </Text>
            </View>
            <View style={styles.roleBadge}>
              <Building2 size={11} color="#ea580c" style={{ marginRight: 4 }} />
              <Text style={styles.roleBadgeText}>RESPONSABLE ONG</Text>
            </View>
          </View>

          <Text style={[styles.userName, { color: colors.text }]}>{fullName}</Text>
          <Text style={[styles.userEmail, { color: colors.textSecondary }]}>
            {userContext?.email || 'responsable.ong@tila.ci'}
          </Text>

          <View style={[styles.divider, { backgroundColor: colors.border }]} />

          <View style={styles.infoRow}>
            <Building2 size={16} color="#ea580c" style={{ marginRight: 10 }} />
            <Text style={[styles.infoLabel, { color: colors.textSecondary }]}>Organisation :</Text>
          ...
          {userContext?.phone ? (
            <View style={styles.infoRow}>
              <Phone size={16} color="#ea580c" style={{ marginRight: 10 }} />
